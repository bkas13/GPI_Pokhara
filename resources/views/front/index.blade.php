...
@section('content')
                ...
                <div class="col-md-7">
                    <div class="about-wrapper">
                        <div class="main-title">
                            {{ getFrontLanguage('welcome-to') }}
                            "{{ isset($settings['name']) ? $settings['name'] : 'High School' }}"
                        </div>
                        <div class="description" style="font-size: 16px; line-height: 30px;">
                                ...
                                @foreach ($context->block_notices as $notice)
                                    <div>
                                        <div class="item-wrapper">
                                            <div class="detail">
                                                <a class="title"
                                                    href="{{ route('front.singleNotice', [$notice->id, getNepaliDate($notice->created_at)]) }}">
                                                    {{ $notice->title }}
                                                </a>
                                                <div class="date">
                        ...
                        <ul class="nav nav-tabs">
                            <li class="nav-item">
                                <a class="nav-link active" data-toggle="tab"
                                    href="#home">{{ getFrontLanguage('news-1') }}</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link" data-toggle="tab"
                                    href="#menu1">{{ getFrontLanguage('tender-1') . ' ' . getFrontLanguage('notice-1') }}</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link" data-toggle="tab" href="#menu2">{{ getFrontLanguage('event') }}</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link" data-toggle="tab"
                                    href="#menu3">{{ getFrontLanguage('scholarship-1') }}</a>
                            </li>
                        </ul>
                                    ...
                                    @if ($context->news->isNotEmpty())
                                        @foreach ($context->news as $news)
                                            <div class="publication-item">
                                                <a class="title"
                                                    href="{{ route('front.singleNews', [$news->id, getNepaliDate($news->created_at)]) }}">
                                                    {{ $news->title }}
                                                </a>
                                                <div class="date">
                    ...
                    <div class="officer-container">
                        ...
                        @foreach ($context->messages as $message)
                            <div class="officer-item">
                                <div class="img-container">
                                    {{-- <img src="{{ asset(isset($settings['adhyaksh_image']) ? 'thumbnail/' . $settings['adhyaksh_image'] : '') }}"
                                        alt="" class="img-fluid" /> --}}
                                ...
                                <div class="info">
                                    <div class="name">
                                        {{-- {{ isset($settings['adhyaksh_name']) ? $settings['adhyaksh_name'] : '' }} --}}
                                        {{ $message->name }}
                                    </div>
                                    {{-- <div class="designation"> {{ getFrontLanguage('ward-chairman') }}</div> --}}
                                    <div class="designation"> {{ $message->designation }}</div>
                                </div>
                            </div>
                        @endforeach
                            ...
                            <div class="col-md-4">
                                <div class="album-container">
                                    <a href="{{ route('front.singleAlbum', $album->slug) }}">
                                        <div class="img-container">
                                            <img src="{{ asset(isset($album->gallerys) ? $album->gallerys->first()->image : 'front/assets/images/banner1.jpg') }}"
                                                alt="{{ $album->title }}" />
                                        </div>
                                        <div class="overlay">
                                            <div class="view">
                ...
                @foreach ($context->staffs as $staff)
                    <div>
                        <div class="facility-item">
                            <div class="img-container">
                                <img src="{{ asset('thumbnail/' . $staff->image) }}" alt="" class="img-fluid" />
                            </div>
                ...
                <div class="col-md-4">
                    <div class="counter">
                        <h2 class="timer count-title count-number"
                            ...
                            data-speed="3000">
                            {{ isset($settings['total_administrations']) ? $settings['total_administrations'] : '' }}
                        </h2>
                ...
                <div class="col-md-4">
                    <div class="counter">
                        <h2 class="timer count-title count-number"
                            ...
                            data-speed="3000">{{ isset($settings['total_teachers']) ? $settings['total_teachers'] : '' }}
                        </h2>
                ...
                <div class="col-md-4">
                    <div class="counter">
                        <h2 class="timer count-title count-number"
                            ...
                            data-speed="3000">{{ isset($settings['total_student']) ? $settings['total_student'] : '' }}
                        </h2>
    ...
    @if (isset($popups))
        @foreach ($popups as $popup)
            <div class="modal fade popup-container" id="exampleModal" tabindex="-1" role="dialog"
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content" style="height: 80vh; overflow-y:scroll;">
                        ...
                        <div class="modal-body">
                            <a target="_blank" href="{{ asset($popup->image) }}">
