...
class IndexController extends Controller
{
    public function home()
    {
        ...
        $context->messages = Messages::orderBy('priority', 'asc')->take(2)->get();

        //        $assetCategories = AssetCategory::where('s chool_id',$school_id)->get(['id','title']);
    ...
    {
        // $messages = Messages::orderBy('priority','asc')->take(2)->get();
        $messages = Messages::orderBy('priority', 'asc')->get();
        return view('front.school.principal-note', compact('messages'));
    }
    ...
    {
        $school_id = 1;
        $staffs = BoardOfDirector::all();
        $recent_notices = Notice::where('school_id', $school_id)->where('status', 1)->orderBy('created_at', 'desc')->get()->take(5);
        return view('front.members.boardofdirectors', compact('staffs', 'recent_notices'));
    }

    public function boardofdirectordetail($id)
    {
        $staff = BoardOfDirector::where('id', $id)->first();
        return view('front.members.boardofdirectordetail', compact('staff'));
    }
}
